...
export const Badge = ({ children }) => (
  <span className={`badge ${String(children || '').toLowerCase().replaceAll('_', '-')}`}>
...
export function Empty({ title = 'Nothing to show', children }) {
  return (
    <div className="empty">
      <b>□</b>
      <h3>{title}</h3>
...
export function ErrorState({ text, retry }) {
  return (
    <div className="error-state">
      <h3>We could not load this data</h3>
      <p>{text}</p>
...
export function ConfirmDialog({ title, message, action, tone = 'danger', onClose }) {
  return (
    <Modal title={title} onClose={onClose}>
      <p>{message}</p>
      <div className="dialog-actions">
        <button className="outline" onClick={onClose}>Cancel</button>
        <button className={tone} onClick={action}>Confirm</button>
      </div>
    </Modal>
  );
}

export function Stat({ label, value, tone = 'blue', detail }) {
  return (
...
export function Timeline({ status }) {
  const steps = ['DRAFT', 'SUBMITTED', 'ASSIGNED', 'IN_VERIFICATION', 'CERTIFICATE_ISSUED'];
  const active = Math.max(0, steps.indexOf(status));
  return (
    <div className="timeline" aria-label={`Application status: ${status}`}>
